...
static int _fetch_headers(OggVorbis_File *vf,vorbis_info *vi,vorbis_comment *vc,
  ...
  ivorbis_info_init(vi);
  ivorbis_comment_init(vc);
  ...
  while(ogg_page_bos(og_ptr)){
    ...
    if(vf->ready_state<STREAMSET){
      ...
      if(ogg_stream_packetout(&vf->os,&op) > 0 &&
         ivorbis_synthesis_idheader(&op)){
        /* vorbis header; continue setup */
        vf->ready_state=STREAMSET;
        if((ret=ivorbis_synthesis_headerin(vi,vc,&op))){
          ret=OV_EBADHEADER;
          goto bail_header;
        }
      }
 ...
 bail_header:
  ivorbis_info_clear(vi);
  ivorbis_comment_clear(vc);
  vf->ready_state=OPENED;
  ...
  while(1){
    ...
    while((result=ogg_stream_packetout(&vf->os,&op))){
      if(result>0){ /* ignore holes */
        long thisblock=ivorbis_packet_blocksize(vi,&op);
        if(lastblock!=-1)
...
static int _make_decode_ready(OggVorbis_File *vf){
  ...
  if(vf->seekable){
    if(ivorbis_synthesis_init(&vf->vd,vf->vi+vf->current_link))
      return OV_EBADLINK;
  }else{
    if(ivorbis_synthesis_init(&vf->vd,vf->vi))
      return OV_EBADLINK;
  }
  ivorbis_block_init(&vf->vd,&vf->vb);
...
static int _open_seekable2(OggVorbis_File *vf){
  ...
  return(iov_raw_seek(vf,dataoffset));
}

/* clear out the current logical bitstream decoder */
static void _decode_clear(OggVorbis_File *vf){
  ivorbis_dsp_clear(&vf->vd);
  ivorbis_block_clear(&vf->vb);
  vf->ready_state=OPENED;
}
    ...
    if(vf->ready_state==INITSET){
      while(1) {
        ...
        if(result>0){
          ...
          if(!ivorbis_synthesis(&vf->vb,op_ptr)){ /* lazy check for lazy
                                                    header handling.  The
            ...
            {
              int oldsamples=ivorbis_synthesis_pcmout(&vf->vd,NULL);
              /* for proper use of libvorbis within libvorbisfile,
                 oldsamples will always be zero. */
              if(oldsamples)return(OV_EFAULT);

              ivorbis_synthesis_blockin(&vf->vd,&vf->vb);
              vf->samptrack+=ivorbis_synthesis_pcmout(&vf->vd,NULL)-oldsamples;
            ...
            if(granulepos!=-1 && !op_ptr->e_o_s){
              ...
              samples=ivorbis_synthesis_pcmout(&vf->vd,NULL);
        ...
        if(vf->ready_state==INITSET){
          if(vf->current_serialno!=ogg_page_serialno(&og)){
            ...
            if(ogg_page_bos(&og)){
              ...
              if(!vf->seekable){
                ivorbis_info_clear(vf->vi);
                ivorbis_comment_clear(vf->vc);
              }
              break;
...
int iov_clear(OggVorbis_File *vf){
  if(vf){
    ivorbis_block_clear(&vf->vb);
    ivorbis_dsp_clear(&vf->vd);
    ...
    if(vf->vi && vf->links){
      ...
      for(i=0;i<vf->links;i++){
        ivorbis_info_clear(vf->vi+i);
        ivorbis_comment_clear(vf->vc+i);
      }
      _ogg_free(vf->vi);
      _ogg_free(vf->vc);
    }
...
int iov_open_callbacks(void *f,OggVorbis_File *vf,
...
int iov_open(FILE *f,OggVorbis_File *vf,const char *initial,long ibytes){
  ...
  return iov_open_callbacks((void *)f, vf, initial, ibytes, callbacks);
}

int iov_fopen(const char *path,OggVorbis_File *vf){
  int ret;
  FILE *f = fopen(path,"rb");
  if(!f) return -1;

  ret = iov_open(f,vf,NULL,0);
  if(ret) fclose(f);
  return ret;
}
...
int iov_test_callbacks(void *f,OggVorbis_File *vf,
    const char *initial,long ibytes,ov_callbacks callbacks)
{
  return _ov_open1(f,vf,initial,ibytes,callbacks);
}

int iov_test(FILE *f,OggVorbis_File *vf,const char *initial,long ibytes){
  ov_callbacks callbacks = {
    (size_t (*)(void *, size_t, size_t, void *))  fread,
    (int (*)(void *, ogg_int64_t, int))              _fseek64_wrap,
    (int (*)(void *))                             fclose,
  ...
  return iov_test_callbacks((void *)f, vf, initial, ibytes, callbacks);
}

int iov_test_open(OggVorbis_File *vf){
  if(vf->ready_state!=PARTOPEN)return(OV_EINVAL);
  return _ov_open2(vf);
}

/* How many logical bitstreams in this physical bitstream? */
long iov_streams(OggVorbis_File *vf){
  return vf->links;
}

/* Is the FILE * associated with vf seekable? */
long iov_seekable(OggVorbis_File *vf){
  return vf->seekable;
}
...
long iov_bitrate(OggVorbis_File *vf,int i){
  if(vf->ready_state<OPENED)return(OV_EINVAL);
  if(i>=vf->links)return(OV_EINVAL);
  if(!vf->seekable && i!=0)return(iov_bitrate(vf,0));
  if(i<0){
    ogg_int64_t bits=0;
    int i;
    for(i=0;i<vf->links;i++)
     ...
     */
    return(bits*1000/iov_time_total(vf,-1));
  }else{
    if(vf->seekable){
      /* return the actual bitrate */
      return((vf->offsets[i+1]-vf->dataoffsets[i])*8000/iov_time_total(vf,i));
    }else{
...
*/
long iov_bitrate_instant(OggVorbis_File *vf){
  int link=(vf->seekable?vf->current_link:0);
  long ret;
  if(vf->ready_state<OPENED)return(OV_EINVAL);
...
/* Guess */
long iov_serialnumber(OggVorbis_File *vf,int i){
  if(i>=vf->links)return(iov_serialnumber(vf,vf->links-1));
  if(!vf->seekable && i>=0)return(iov_serialnumber(vf,-1));
  if(i<0){
...
*/
ogg_int64_t iov_raw_total(OggVorbis_File *vf,int i){
  if(vf->ready_state<OPENED)return(OV_EINVAL);
...
*/
ogg_int64_t iov_pcm_total(OggVorbis_File *vf,int i){
  if(vf->ready_state<OPENED)return(OV_EINVAL);
  if(!vf->seekable || i>=vf->links)return(OV_EINVAL);
  if(i<0){
    ogg_int64_t acc=0;
...
/* returns: total milliseconds of content if i==-1
            ...
            length) or only partially open 
*/
ogg_int64_t iov_time_total(OggVorbis_File *vf,int i){
  if(vf->ready_state<OPENED)return(OV_EINVAL);
  if(!vf->seekable || i>=vf->links)return(OV_EINVAL);
  if(i<0){
    ogg_int64_t acc=0;
...
int iov_raw_seek(OggVorbis_File *vf,ogg_int64_t pos){
  ...
  ivorbis_synthesis_restart(&vf->vd);
...

  {
    ...
    while(1){
      if(vf->ready_state>=STREAMSET){
        ...
        if(result>0){
          ...
          if(vf->vi[vf->current_link].codec_setup){
            thisblock=ivorbis_packet_blocksize(vf->vi+vf->current_link,&op);
            if(thisblock<0){
      ...
      if(!lastblock){
        ...
        if(pagepos<0){
          vf->pcm_offset=iov_pcm_total(vf,-1);
          break;
        }
      }else{
...
ogg_int64_t _irescale64(ogg_int64_t x, ogg_int64_t from, ogg_int64_t to){
  ogg_int64_t frac=0;
  ogg_int64_t ret=0;
  int i;
  if(x >= from) return to;
...
int iov_pcm_seek_page(OggVorbis_File *vf,ogg_int64_t pos){
  int link=-1;
  ogg_int64_t result=0;
  ogg_int64_t total=iov_pcm_total(vf,-1);
  ...
  /* new search algorithm by HB (Nicholas Vinen) */
  {
    ...
    while(begin<end){
      ...
      if(end-begin<CHUNKSIZE){
        ...
        bisect=begin + _irescale64(target-begintime,
				 endtime-begintime,
				 end-begin) - CHUNKSIZE;
  ...
  /* verify result */
  if(vf->pcm_offset>pos || pos>iov_pcm_total(vf,-1)){
    result=OV_EFAULT;
    goto seek_error;
  }
  vf->bittrack=0;
...
int iov_pcm_seek(OggVorbis_File *vf,ogg_int64_t pos){
  int thisblock,lastblock=0;
  int ret=iov_pcm_seek_page(vf,pos);
  if(ret<0)return(ret);
  ...
  while(1){
    ...
    if(ret>0){
      ...
      if(vf->pcm_offset+((thisblock+
                          ivorbis_info_blocksize(vf->vi,1))>>2)>=pos)break;

      /* remove the packet from packet queue and track its granulepos */
      ogg_stream_packetout(&vf->os,NULL);
      ivorbis_synthesis_trackonly(&vf->vb,&op);  /* set up a vb with
                                                   only tracking, no
                                                   pcm_decode */
      ivorbis_synthesis_blockin(&vf->vd,&vf->vb);
  ...
  while(vf->pcm_offset<pos){
    ...
    long samples=ivorbis_synthesis_pcmout(&vf->vd,NULL);

    if(samples>target)samples=target;
    ivorbis_synthesis_read(&vf->vd,samples);
    vf->pcm_offset+=samples;

    if(samples<target)
      if(_fetch_and_process_packet(vf,NULL,1,1)<=0)
        vf->pcm_offset=iov_pcm_total(vf,-1); /* eof */
  }
  return 0;
}
...
/* seek to a playback time relative to the decompressed pcm stream
   returns zero on success, nonzero on failure */
int iov_time_seek(OggVorbis_File *vf,ogg_int64_t milliseconds){
  ...
  for(link=0;link<vf->links;link++){
    ogg_int64_t addsec = iov_time_total(vf,link);
  ...
  /* enough information to convert time offset to pcm offset */
  {
    ...
    return(iov_pcm_seek(vf,target));
  }
}

/* page-granularity version of ov_time_seek 
   returns zero on success, nonzero on failure */
int iov_time_seek_page(OggVorbis_File *vf,ogg_int64_t milliseconds){
  /* translate time to PCM position and call ov_pcm_seek */
  ...
  {
    ogg_int64_t target=pcm_total+(milliseconds-time_total)*vf->vi[link].rate/1000;
    return(iov_pcm_seek_page(vf,target));
  }
}

/* tell the current stream offset cursor.  Note that seek followed by
   tell will likely not give the set offset due to caching */
ogg_int64_t iov_raw_tell(OggVorbis_File *vf){
  if(vf->ready_state<OPENED)return(OV_EINVAL);
  return(vf->offset);
}

/* return PCM offset (sample) of next PCM sample to be read */
ogg_int64_t iov_pcm_tell(OggVorbis_File *vf){
  if(vf->ready_state<OPENED)return(OV_EINVAL);
  return(vf->pcm_offset);
}

/* return time offset (milliseconds) of next PCM sample to be read */
ogg_int64_t iov_time_tell(OggVorbis_File *vf){
  int link=0;
  ogg_int64_t pcm_total=0;
  ogg_int64_t time_total=0;
  ...
  if(vf->ready_state<OPENED)return(OV_EINVAL);
  if(vf->seekable){
    pcm_total=iov_pcm_total(vf,-1);
    time_total=iov_time_total(vf,-1);

    /* which bitstream section does this time offset occur in? */
    for(link=vf->links-1;link>=0;link--){
      pcm_total-=vf->pcmlengths[link*2+1];
      time_total-=iov_time_total(vf,link);
...
vorbis_info *iov_info(OggVorbis_File *vf,int link){
  if(vf->seekable){
    if(link<0)
      if(vf->ready_state>=STREAMSET)
...
vorbis_comment *iov_comment(OggVorbis_File *vf,int link){
  if(vf->seekable){
    if(link<0)
      if(vf->ready_state>=STREAMSET)
...
long iov_read(OggVorbis_File *vf,char *buffer,int bytes_req,int *bitstream){
  int i,j;
  ...
  while(1){
    if(vf->ready_state==INITSET){
      samples=ivorbis_synthesis_pcmout(&vf->vd,&pcm);
      if(samples)break;
    }
  ...
  if(samples>0){
    ...
    long channels=iov_info(vf,-1)->channels;
