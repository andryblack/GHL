...
void ivorbis_lsp_to_curve(ogg_int32_t *curve,int *map,int n,int ln,
...
static void *floor0_inverse1(vorbis_block *vb,vorbis_look_floor *i){
  ...
  if(ampraw>0){ /* also handles the -1 out of data case */
    ...
    if(booknum!=-1 && booknum<info->numbooks){ /* be paranoid */
      ...
      ogg_int32_t last=0;
      ogg_int32_t *lsp=(ogg_int32_t *)_ivorbis_block_alloc(vb,sizeof(*lsp)*(look->m+1));
            
      if(ivorbis_book_decodev_set(b,lsp,&vb->opb,look->m,-24)==-1)goto eop;
      for(j=0;j<look->m;){
...
static int floor0_inverse2(vorbis_block *vb,vorbis_look_floor *i,
  ...
  if(memo){
    ...
    ivorbis_lsp_to_curve(out,look->linearmap,look->n,look->ln,
...
/* export hooks */
vorbis_func_floor ifloor0_exportbundle={
