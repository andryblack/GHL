...
ogg_uint32_t *_imake_words(long *l,long n,long sparsecount){
  long i,j,count=0;
...
long _ibook_maptype1_quantvals(const static_codebook *b){
...
ogg_int32_t *_ibook_unquantize(const static_codebook *b,int n,int *sparsemap,
			      int *maxpoint){
  long j,k,count=0;
  if(b->maptype==1 || b->maptype==2){
    ...
    *maxpoint=minpoint;
    ...
    case 1:
	 ...
	 that */
      quantvals=_ibook_maptype1_quantvals(b);
...
void ivorbis_staticbook_destroy(static_codebook *b){
...
void ivorbis_book_clear(codebook *b){
...
int ivorbis_book_init_decode(codebook *c,const static_codebook *s){
  int i,j,n=0,tabn;
  int *sortindex;
  memset(c,0,sizeof(*c));
  ...
  if(n>0){
    ...
    /* perform sort */
    ogg_uint32_t *codes=_imake_words(s->lengthlist,s->entries,c->used_entries);
    ...
    c->valuelist=_ibook_unquantize(s,n,sortindex,&c->binarypoint);
