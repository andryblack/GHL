...
static void *floor1_inverse1(vorbis_block *vb,vorbis_look_floor *in){
  ...
  if(oggpack_read(&vb->opb,1)==1){
    int *fit_value=(int *)_ivorbis_block_alloc(vb,(look->posts)*sizeof(*fit_value));
    ...
    for(i=0,j=2;i<info->partitions;i++){
      ...
      if(csubbits){
	cval=ivorbis_book_decode(books+info->class_book[classv],&vb->opb);
	...
	cval>>=csubbits;
	if(book>=0){
	  if((fit_value[j+k]=ivorbis_book_decode(books+book,&vb->opb))==-1)
	    goto eop;
	}else{
	  fit_value[j+k]=0;
	}
...
/* export hooks */
vorbis_func_floor ifloor1_exportbundle={
